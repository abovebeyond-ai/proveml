...
export function reviewPage(opts) {
  ...
  <div class="cols">
    ...
    if (manIds.length) {
        ...
        // anchors.hedera is the package's path (proveml/anchor); rekor and vana
        // are records from the demo adapters, rendered when a build passes them.
        ...
        const rowHtml = (r, withGroup = false) => `<span class="n">${esc(sourceTitles[r.id] || r.id)}${withGroup && r.group ? `<small> ${esc(r.group)}</small>` : ''}</span><span class="m">${esc(r.words.join(', '))}</span><span class="c">${r.n} ${r.n === 1 ? 'reading rests on it' : 'readings rest on it'}</span>${r.note ? `<span class="let">${esc(r.note)}</span>` : ''}`;
        const backed = `<h2>Is it backed?</h2><p class="bh-answer">Every claim rests on one of <b>${manIds.length}</b> sources. Vera holds a copy of each; the words in green say what else vouches for it.${weak.length ? ` <b data-k="let">${weak.length} to look at</b>, first.` : ' Nothing to look at.'}</p><div class="bh-register"><span class="kop">source</span><span class="kop m">vouched for by</span><span class="kop"></span>${weak.map((r) => rowHtml(r, true)).join('')}${groups.map((g) => { const ids = rows.filter((r) => strong.includes(r) && g.ids.includes(r.id)); if (!ids.length) return ''; return `<span class="groep">${esc(g.title)}<small>${ids.length}</small></span>` + ids.sort((a, b) => b.n - a.n).map(rowHtml).join(''); }).join('')}</div><p class="bh-note">Copy: Vera holds the text. Checked live: fetched again over TLS, the same. Archived: an independent copy exists. Timestamped: an authority vouches for when. Reproduced: the script gave the same bytes. Under a grant: read from its owner on their terms.</p>`;
