...
const BRIDGE = `
<style>#rv-handback:not(:disabled){color:var(--accent)}#rv-handback:not(:disabled):hover{color:var(--ink)}</style>
<script>
(async () => {
    ...
    btn.id = 'rv-handback'; btn.className = 'rv-act';
    ...
    const state = () => {
        ...
        if (handedBack()) { btn.disabled = true; btn.textContent = 'with Vera \\u2713'; }
        else if (open) { btn.disabled = true; btn.textContent = open + ' to judge, then hand back'; }
        else { btn.disabled = false; btn.textContent = 'hand back to Vera'; }
    };
